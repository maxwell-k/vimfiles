<!-- vim: set filetype=markdown.markdown-toc : -->
<!--
test/manual/README.md
Copyright 2020 Keith Maxwell
SPDX-License-Identifier: CC-BY-SA-4.0
-->

# Manual tests

<!-- toc -->

- [Focus mode](#focus-mode)
- [`gopass` syntax highlighting](#gopass-syntax-highlighting)
- [Markdown syntax highlighting](#markdown-syntax-highlighting)
- [todo syntax highlighting](#todo-syntax-highlighting)

<!-- tocstop -->

## Beancount

Command to view example journal

    vim test/manual/test.beancount

## Focus mode

Command to view example focus file:

    vim test/manual/focus/todo.txt

Command to view example `todo.txt` file in focus mode::

    vim "+set filetype=focus" test/manual/focus/todo.txt

## `gopass` syntax highlighting

Command to view example `gopass`:

    vim "+set filetype=focus" test/manual/focus/todo.txt

## Markdown syntax highlighting

Command to view example Markdown file:

    vim test/manual/example.md

## todo syntax highlighting

Command to view example [todo] file:

    vim test/manual/todo.txt

[todo]: http://todotxt.org/
